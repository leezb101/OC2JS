...
-(void)drawRect:(CGRect)rect{
    ...
    CGColorSpaceRelease(baseSpace); // Add by lisz5
    ...
    CGGradientRelease(gradient);// Add by lisz5
...
- (void)bind:(ALAsset *)asset selectionFilter:(NSPredicate*)selectionFilter isSeleced:(BOOL)isSeleced
{
    ...
    // Modify By lisz5 （imageWithCGImage 耗时操作放在异步线程进行）
    __weak ALAsset * weakAsset = asset;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        @autoreleasepool {
            UIImage * image = [UIImage imageWithCGImage:weakAsset.thumbnail];
...
- (void)setupButtons
{
    ...
    if([[[UIDevice currentDevice]systemVersion]doubleValue]>=7.0){
        self.navigationItem.rightBarButtonItem.tintColor = RGBCOLOR(0x3b, 0x3d, 0x41);//[UIColor whiteColor];
          //  [[UINavigationBar appearance]setTintColor:[UIColor whiteColor]];
        self.navigationItem.leftBarButtonItem.tintColor = RGBCOLOR(0x3b, 0x3d, 0x41);//[UIColor whiteColor];
    }
}
...
- (void)setNavTitle:(NSString *)title{
    UIFont * font = [UIFont fontWithName:@"FZLanTingHei-L-GBK" size:16];
    UILabel *titleLable = [Tools createLabel: title
                                       frame:CGRectMake(0, 0, 200, 30)
                                       color:RGBCOLOR(0x3b, 0x3d, 0x41)
                                        font:font
...
- (void)setupButtons
{
    ...
    if (picker.showCancelButton)
    {
        ...
        [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"取消", nil)
            ...
            self.navigationItem.rightBarButtonItem.tintColor = RGBCOLOR(0xee, 0x77, 0x15);
    }
}

- (void)setNavTitle:(NSString *)title{
    UIFont * font = [UIFont fontWithName:@"FZLanTingHei-L-GBK" size:17];
    UILabel *titleLable = [Tools createLabel: title
                                       frame:CGRectMake(0, 0, 200, 30)
                                       color:RGBCOLOR(0x3b, 0x3d, 0x41)
                                        font:font
...
- (void)dealloc {
    NSLog(@"ZYQAssetPickerController dealloc");
}
